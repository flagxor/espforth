...
// Setup timer and attach timer to a led pin
  ...
  HEADER("NOP");
  int NOP=CODE(as_nop, as_next);
  HEADER("ACCEPT");
  int ACCEP=CODE(as_accept, as_next);
  HEADER("?KEY");
  int QKEY=CODE(as_qrx, as_next);
  HEADER("EMIT");
  int EMIT=CODE(as_txsto, as_next);
  HEADER("DOLIT");
  int DOLIT=CODE(as_dolit, as_next);
  HEADER("DOLIST");
  int DOLST=CODE(as_dolist, as_next);
  HEADER("EXIT");
  int EXITT=CODE(as_exit, as_next);
  HEADER("EXECUTE");
  int EXECU=CODE(as_execu, as_next);
  HEADER("DONEXT");
  DONXT=CODE(as_donext, as_next);
  HEADER("QBRANCH");
  QBRAN=CODE(as_qbran, as_next);
  HEADER("BRANCH");
  BRAN=CODE(as_bran, as_next);
  HEADER("!");
  int STORE=CODE(as_store, as_next);
  HEADER("@");
  int AT=CODE(as_at, as_next);
  HEADER("C!");
  int CSTOR=CODE(as_cstor, as_next);
  HEADER("C@");
  int CAT=CODE(as_cat, as_next);
  HEADER("R>");
  int RFROM=CODE(as_rfrom, as_next);
  HEADER("R@");
  int RAT=CODE(as_rat, as_next);
  HEADER(">R");
  TOR=CODE(as_tor, as_next);
  HEADER("DROP");
  int DROP=CODE(as_drop, as_next);
  HEADER("DUP");
  int DUPP=CODE(as_dup, as_next);
  HEADER("SWAP");
  int SWAP=CODE(as_swap, as_next);
  HEADER("OVER");
  int OVER=CODE(as_over, as_next);
  HEADER("0<");
  int ZLESS=CODE(as_zless, as_next);
  HEADER("AND");
  int ANDD=CODE(as_andd, as_next);
  HEADER("OR");
  int ORR=CODE(as_orr, as_next);
  HEADER("XOR");
  int XORR=CODE(as_xorr, as_next);
  HEADER("UM+");
  int UPLUS=CODE(as_uplus, as_next);
  HEADER("?DUP");
  int QDUP=CODE(as_qdup, as_next);
  HEADER("ROT");
  int ROT=CODE(as_rot, as_next);
  HEADER("2DROP");
  int DDROP=CODE(as_ddrop, as_next);
  HEADER("2DUP");
  int DDUP=CODE(as_ddup, as_next);
  HEADER("+");
  int PLUS=CODE(as_plus, as_next);
  HEADER("NOT");
  int INVER=CODE(as_inver, as_next);
  HEADER("NEGATE");
  int NEGAT=CODE(as_negat, as_next);
  HEADER("DNEGATE");
  int DNEGA=CODE(as_dnega, as_next);
  HEADER("-");
  int SUBBB=CODE(as_subb, as_next);
  HEADER("ABS");
  int ABSS=CODE(as_abss, as_next);
  HEADER("=");
  int EQUAL=CODE(as_equal, as_next);
  HEADER("U<");
  int ULESS=CODE(as_uless, as_next);
  HEADER("<");
  int LESS=CODE(as_less, as_next);
  HEADER("UM/MOD");
  int UMMOD=CODE(as_ummod, as_next);
  HEADER("M/MOD");
  int MSMOD=CODE(as_msmod, as_next);
  HEADER("/MOD");
  int SLMOD=CODE(as_slmod, as_next);
  HEADER("MOD");
  int MODD=CODE(as_mod, as_next);
  HEADER("/");
  int SLASH=CODE(as_slash, as_next);
  HEADER("UM*");
  int UMSTA=CODE(as_umsta, as_next);
  HEADER("*");
  int STAR=CODE(as_star, as_next);
  HEADER("M*");
  int MSTAR=CODE(as_mstar, as_next);
  HEADER("*/MOD");
  int SSMOD=CODE(as_ssmod, as_next);
  HEADER("*/");
  int STASL=CODE(as_stasl, as_next);
  HEADER("PICK");
  int PICK=CODE(as_pick, as_next);
  HEADER("+!");
  int PSTOR=CODE(as_pstor, as_next);
  HEADER("2!");
  int DSTOR=CODE(as_dstor, as_next);
  HEADER("2@");
  int DAT=CODE(as_dat, as_next);
  HEADER("COUNT");
  int COUNT=CODE(as_count, as_next);
  HEADER("MAX");
  int MAX=CODE(as_max, as_next);
  HEADER("MIN");
  int MIN=CODE(as_min, as_next);
  HEADER("BL");
  int BLANK=CONSTANT(32);
  HEADER("CELL");
  int CELL=CONSTANT(sizeof(cell_t));
  HEADER("CELL+");
  int CELLP=CODE(as_docon, as_plus, as_next); Comma(sizeof(cell_t));
  HEADER("CELL-");
  int CELLM=CODE(as_docon, as_subb, as_next); Comma(sizeof(cell_t));
  HEADER("CELLS");
  int CELLS=CODE(as_docon, as_star, as_next); Comma(sizeof(cell_t));
  HEADER("CELL/");
  int CELLD=CODE(as_docon, as_slash, as_next); Comma(sizeof(cell_t));
  HEADER("1+");
  int ONEP=CODE(as_docon, as_plus, as_next); Comma(1);
  HEADER("1-");
  int ONEM=CODE(as_docon, as_subb, as_next); Comma(1);
  HEADER("2+");
  int TWOP=CODE(as_docon, as_plus, as_next); Comma(2);
  HEADER("2-");
  int TWOM=CODE(as_docon, as_subb, as_next); Comma(2);
  HEADER("2*");
  int TWOST=CODE(as_docon, as_star, as_next); Comma(2);
  HEADER("2/");
  int TWOS=CODE(as_docon, as_slash, as_next); Comma(2);
  HEADER("sendPacket");
  int SENDP=CODE(as_sendPacket, as_next);
  HEADER("POKE");
  int POKE=CODE(as_poke, as_next);
  HEADER("PEEK");
  int PEEK=CODE(as_peek, as_next);
  HEADER("ADC");
  int ADC=CODE(as_adc, as_next);
  HEADER("PIN");
  int PIN=CODE(as_pin, as_next);
  HEADER("TONE");
  int TONE=CODE(as_tone,as_next);
  HEADER("DUTY");
  int DUTY=CODE(as_duty, as_next);
  HEADER("FREQ");
  int FREQ=CODE(as_freq, as_next);
  HEADER("MS");
  int MS=CODE(as_ms, as_next);
